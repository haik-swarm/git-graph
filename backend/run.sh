...
# A restart triggered from an agent shell inherits that shell's PYTHONPATH,
# which points at the OpenSwarm app's bundled 3.13 site-packages. Those win
# over this venv's own 3.11 packages, so pydantic resolves to a build whose
# compiled extension can't load. The venv is self-contained; clear it.
unset PYTHONPATH
unset PYTHONHOME
